...
def plasma_fractal(mapsize=256, wibbledecay=3):
    ...
    maparray = np.empty((mapsize, mapsize))
    maparray[0, 0] = 0
    stepsize = mapsize
    wibble = 100
    ...
    for i in range(c[2]):
        for h in range(height - c[1], c[1], -1):
            for w in range(width - c[1], c[1], -1):
                ...
                h_prime = np.clip(h + dy, 0, height - 1)
                w_prime = np.clip(w + dx, 0, width - 1)
                # swap
...
def fog(x, severity=1):
    ...
    # 生成与输入图像尺寸一致的 plasma noise
    fog_map = plasma_fractal(wibbledecay=c[1], mapsize=max(h, w))

    # 缩放到目标大小并扩展维度
    fog_map = fog_map[:h, :w][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    if frost is None:
        raise FileNotFoundError("无法加载任何 frost 图像，请检查文件路径和完整性。")
    ...
    # 随机裁剪一个 224x224 的区域
    try:
        x_start = np.random.randint(0, frost.shape[0] - 224 + 1)
        y_start = np.random.randint(0, frost.shape[1] - 224 + 1)
    except ValueError:
        raise ValueError("frost 图像尺寸不足 224x224，请使用更大尺寸的背景图")

    frost = frost[x_start:x_start+224, y_start:y_start+224]

    # 缩放为输入图像大小
    frost = cv2.resize(frost, (w, h), interpolation=cv2.INTER_AREA)
...
def snow(x, severity=1):
    ...
    # 创建雪层
    snow_layer = np.random.normal(size=(h, w), loc=c[0], scale=c[1])  # 使用与输入一致的宽高
    ...
    snow_layer = snow_layer[..., np.newaxis]  # 添加通道维度

    # 自动适配灰度图尺寸
    gray = cv2.cvtColor((x * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.
    gray = gray.reshape(h, w, 1)  # 保证形状匹配
...
def spatter(x, severity=1):
    ...
    if c[5] == 0:
        ...
        m /= np.max(m, axis=(0, 1), keepdims=True) + 1e-6
        m *= c[4]

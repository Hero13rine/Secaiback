...
本文面向需要在本项目中扩展自定义模型与数据加载逻辑的开发者，介绍如何将模型封装为可由 `Estimator` 接管的形式，并提供一套可复用的数据加载器模板。
...
### 2.3 组合工厂函数
建议提供一个高层封装函数，同时返回 train/val loader，并将常用配置抽象为参数：
```python
def create_classification_loaders(dataset_root: str, *, batch_size: int, num_workers: int = 4):
    train_loader = build_classification_loader(dataset_root, batch_size=batch_size, train=True, num_workers=num_workers)
    val_loader = build_classification_loader(dataset_root, batch_size=batch_size, train=False, num_workers=num_workers)
